...
fn movement(
    ...
    for event in movement_event_reader.read() {
        for (transform, movement_acceleration, jump_impulse, mut linear_velocity, is_grounded) in
            &mut controllers
        {
            match event {
                MovementAction::Move(direction) => {
                    let normal = -transform.translation.xy().perp().normalize();
                    linear_velocity.0 += *direction * normal * movement_acceleration.0 * delta_time;
                }
                MovementAction::Jump => {
                    if is_grounded {
                        linear_velocity.0 = transform.translation.xy().normalize() * jump_impulse.0;
                    }
                }
            }
        }
